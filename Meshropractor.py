...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
                               ...
                               QSplitter, QColorDialog, QTreeWidget, QTreeWidgetItem,
                               QToolBar, QStyle)
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.setMinimumSize(800, 600)  # Защита от схлопывания окна

        # ВАЖНО: Разрешаем Питону отслеживать мышь, даже когда мы не кликаем
        self.setMouseTracking(True)

        main_widget = QWidget()
        main_widget.setMouseTracking(True)  # Включаем отслеживание и здесь
        # Красим подложку в цвет заголовка, чтобы наша 5px рамка сливалась с ним
        main_widget.setObjectName("MainWidget")
        main_widget.setStyleSheet("#MainWidget { background-color: #2b2b2b; }")

        self.setCentralWidget(main_widget)
        base_layout = QVBoxLayout(main_widget)

        # ТО САМОЕ РЕШЕНИЕ: Делаем отступ в 5 пикселей от краев.
        # Это и есть наша "невидимая рамка" для растягивания.
        base_layout.setContentsMargins(5, 5, 5, 5)

        # === КАСТОМНЫЙ ЗАГОЛОВОК ===
        self.title_bar = QWidget()
        self.title_bar.setCursor(Qt.ArrowCursor)
        self.title_bar.setFixedHeight(40)  # Высота нашего нового заголовка
        self.title_bar.setStyleSheet("background-color: #2b2b2b;")  # Цвет заголовка (темный)

        title_layout = QHBoxLayout(self.title_bar)
        title_layout.setContentsMargins(5, 0, 0, 0)

        # 2. Твоя панель инструментов
        self.toolbar = QToolBar()
        self.toolbar.setStyleSheet("border: none;")

        # === ВШИВАЕМ ЛОГОТИП ===
        # Вставь свою длинную скопированную строку между тройными кавычками!


        # Расшифровываем текст обратно в картинку
        logo_bytes = QByteArray.fromBase64(LOGO_B64)
        logo_pixmap = QPixmap()
        logo_pixmap.loadFromData(logo_bytes)

        self.setWindowIcon(QIcon(logo_pixmap))  # Присваиваем иконку всему окну

        # Создаем рамку (Label) для картинки и сжимаем её до размеров тулбара (30x30)
        logo_label = QLabel()
        logo_label.setPixmap(logo_pixmap.scaled(30, 30, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        # Делаем красивые отступы, чтобы логотип не прилипал к краю и к кнопке сохранения
        logo_label.setStyleSheet("padding-left: 10px; padding-right: 20px;")

        # Добавляем наш логотип в тулбар САМЫМ ПЕРВЫМ
        self.toolbar.addWidget(logo_label)
        # =======================

        # Добавляем кнопки в тулбар (как мы делали раньше)
        icon_save = self.style().standardIcon(QStyle.SP_DialogSaveButton)
        self.action_save = QAction(icon_save, "Сохранить", self)
        self.action_save.triggered.connect(self.save_project)
        self.toolbar.addAction(self.action_save)

        icon_undo = self.style().standardIcon(QStyle.SP_ArrowBack)
        self.toolbar.addAction(QAction(icon_undo, "Назад", self))

        icon_redo = self.style().standardIcon(QStyle.SP_ArrowForward)
        self.toolbar.addAction(QAction(icon_redo, "Вперед", self))

        title_layout.addWidget(self.toolbar)

        # 3. Пружина: растолкает тулбар влево, а кнопки закрытия вправо
        title_layout.addStretch()

        # 4. Создаем свои системные кнопки (Свернуть, Развернуть, Закрыть)
        btn_min = QPushButton("—")
        btn_max = QPushButton("▢")
        btn_close = QPushButton("✕")

        # Красиво их оформляем (при наведении будут подсвечиваться)
        ...
        # Кнопка закрытия при наведении должна быть красной
        btn_close.setStyleSheet("QPushButton { border: none; color: white; font-size: 14px; } "
                                "QPushButton:hover { background-color: #e81123; }")

        # Подключаем их к стандартным функциям окна
        btn_min.clicked.connect(self.showMinimized)
        btn_max.clicked.connect(lambda: self.showNormal() if self.isMaximized() else self.showMaximized())
        btn_close.clicked.connect(self.close)

        # Встраиваем наш заголовок в самый верх программы
        base_layout.addWidget(self.title_bar)

        self.main_splitter = QSplitter(Qt.Horizontal)
        base_layout.addWidget(self.main_splitter)

        # === НОВАЯ ЛЕВАЯ ПАНЕЛЬ (Дерево элементов как в GOM) ===
        left_panel = QWidget()
        left_panel.setCursor(Qt.ArrowCursor)
        left_layout = QVBoxLayout(left_panel)
        left_layout.setContentsMargins(5, 5, 5, 5)

        self.tree = QTreeWidget()
        self.tree.setHeaderLabel("Элементы проекта")  # Заголовок
        self.tree.setStyleSheet("QTreeWidget { font-size: 13px; }")

        # Создаем главные папки (Корневые элементы)
        ...
        self.tree.expandAll()  # Раскрываем все папки при запуске
        left_layout.addWidget(self.tree)

        # Подключаем сигнал: когда юзер кликает на галочку (глазик)
        self.tree.itemChanged.connect(self.on_tree_visibility_changed)

        # --- СРЕДНЯЯ ПАНЕЛЬ (3D Окно) ---
        self.plotter = QtInteractor(self)
        self.plotter.set_background('white')
        self.plotter.add_axes()
        self.plotter.add_key_event('space', self.on_space_pressed)

        # --- ПРАВАЯ ПАНЕЛЬ (Интерфейс) ---
        right_panel = QWidget()
        right_panel.setCursor(Qt.ArrowCursor)
        right_layout = QVBoxLayout(right_panel)
        right_layout.setContentsMargins(5, 5, 5, 5)

        # Добавляем панели в сплиттер строго слева направо!
        self.main_splitter.addWidget(left_panel)
        self.main_splitter.addWidget(self.plotter.interactor)
        self.main_splitter.addWidget(right_panel)

        # Задаем ширину: 250px дереву, 950px 3D-окну, 400px настройкам
    ...
    # --- ЛОГИКА БОКОВОГО ДЕРЕВА ---
    def add_tree_item(self, parent_category, name, actor_key):
        """Удаляет старый файл из папки и добавляет новый с галочкой"""
        # Сначала чистим папку (чтобы не копились старые загрузки)
        parent_category.takeChildren()

        # Создаем новый элемент (Файл) внутри папки
        item = QTreeWidgetItem(parent_category, [name])

        # Включаем для него чекбокс (это будет наш "Глазик" видимости)
        item.setCheckState(0, Qt.Checked)

        # Магия: Прячем внутрь элемента невидимый ключ (например "CAD" или "Scan"),
        # чтобы потом знать, какую 3D-деталь скрывать
    ...
    def on_tree_visibility_changed(self, item, column):
        """Эта функция срабатывает каждый раз, когда юзер ставит/снимает галочку"""
        # Достаем наш спрятанный ключ
        actor_key = item.data(0, Qt.UserRole)

        # Если ключ есть, и такая деталь загружена на 3D-сцену
        if actor_key and self.actors.get(actor_key):
            # Проверяем, стоит ли галочка
            is_visible = (item.checkState(0) == Qt.Checked)

            # Включаем или выключаем видимость в ядре VTK
            self.actors[actor_key].SetVisibility(is_visible)
            self.plotter.render()  # Перерисовываем экран

    # === ЛОГИКА ПЕРЕТАСКИВАНИЯ И РАСТЯГИВАНИЯ ОКНА ===
    def _check_resize_zone(self, pos):
        """Определяет, у какого края находится мышь"""
        x, y = pos.x(), pos.y()
        margin = 6  # Толщина активной зоны захвата (в пикселях)
        dir = ""
        if y < margin:
            dir += "T"  # Top
    ...
    def _update_cursor(self, dir):
        """Меняет системный курсор в зависимости от края"""
        if dir in ["T", "B"]:
    ...
    def mouseMoveEvent(self, event):
        ...
        # Если кнопка мыши НЕ зажата — просто рисуем стрелочки на краях
        if event.buttons() == Qt.NoButton:
            self._update_cursor(self._check_resize_zone(pos))

        # Если зажата на рамке — рассчитываем новую ширину и высоту
            ...
            # Защита от схлопывания (упираемся в минимум)
            if w < 800:
        ...
        # Если зажата на заголовке — двигаем все окно
    ...
    def mouseReleaseEvent(self, event):
        ...
        if hasattr(self, 'drag_pos'):
            del self.drag_pos
        self.setCursor(Qt.ArrowCursor)  # Возвращаем курсор в норму

    # --- ЛОГИКА ВЕРХНЕЙ ПАНЕЛИ ---
    def save_project(self):
        """Функция сохранения"""
        self.log("> СОХРАНЕНИЕ: Функция сохранения проекта пока в разработке...")
        # В будущем здесь можно сохранять сцену в формат .vtk или .ply

    def undo_action(self):
        """Функция шага назад"""
        self.log("> ОТМЕНА: Возврат к предыдущему состоянию")

    def redo_action(self):
        """Функция шага вперед"""
        self.log("> ВОЗВРАТ: Повтор отмененного действия")
    ...
    def load_cad(self):
        ...
        if path:
            ...
            # Кидаем в дерево
            self.add_tree_item(self.cat_cad, filename, "CAD")
    ...
    def load_scan(self):
        ...
        if path:
            ...
            # Кидаем в дерево
            self.add_tree_item(self.cat_scan, filename, "Scan")
    ...
    def on_comp_done(self, result_mesh):
        ...
        # Закидываем результат в папку "Результаты"
        self.add_tree_item(self.cat_res, "Compensated_Part.stl", "Result")

        # Автоматически снимаем галочку (глазик) со Скана в дереве
        if self.cat_scan.childCount() > 0:
            self.cat_scan.child(0).setCheckState(0, Qt.Unchecked)
